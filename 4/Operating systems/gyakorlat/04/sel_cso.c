...
int main() {
    ...
    copy_watchedpipes_read =
        watchedpipes_read; // copy of filedesciptor set - for select
    ...
    // 2.3.4. parameters: set of reading, set of writing, set of exceptions
    // (only for sockets)
    // 5. parameter - timer
    ...
    // select gives back control, when there is some change in any of file
    // descriptors or there is a possibility to read or write at the end of a
    // file, we can read - so it gives back control : (
    ...
    // while the timer shows the time back - it's null now - we have to
    // overwrite!
