...
int main(int argc, char** argv) {
    if (argc != 3) {
        perror("You have to use program with two arguments, the file names "
               "copy_from copy_to");
        exit(1);
    }
    int f, g;
    ...
    // there is an access function with which we can see whethet the file is
    // exist or not access(filename,F_OK); open 1. parameter= file name open 2.
