...
int main() {
    int f;
    char text1[] = " Parent Parent Parent Parent Parent Parent Parent Parent "
                   "Parent Parent ";
    int length1 = strlen(text1);
    char text2[] = " Child Child Child Child Child Child Child Child Child "
                   "Child Child Child ";
    int length2 = strlen(text2);
    int i;
    struct flock lock_data;
    lock_data.l_whence =
        SEEK_SET; // the absolute starting position of locking: SEEK_SET - start
                  // of file, SEEK_END - end of file, SEEK_CUR - actual place of
    lock_data.l_start = 0; // relative position of locking to l_whence
    lock_data.l_len = 0;   // how long is the locked part of the file, 0 - if it
                           // depends on the whole file
    ...
    if (child > 0) { // Parent process
        ...
        for (j = 0; j < 25; j++) // to avoid too big data files
        {
            ...
            lock_data.l_type =
                F_WRLCK; // for what it is locked
                         // F_WRLCK-write,FRDLCK-read,F_UNLCK - unlock
            rc = fcntl(f, F_SETLKW,
                       &lock_data); // F_setlkw - locks file for writing and it
                                    // is waiting, F_SETLK - it locks if it can,
                                    // but not wait
            struct flock l2;
            if (rc != (-1)) {
                for (i = 0; i < length1; i++) {
