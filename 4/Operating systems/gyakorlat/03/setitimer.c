...
int main() {
    ...
    alarm(1); // it is implemented in POSIX systems - but it is adviced to use
              // setitimer
    sleep(2); // waiting the end of alarm BUT usually do not use sleep and alarm
              // or setitimer together - may interference
    printf("\n\n");
    // 2. trial of creating a timer by setitimer function
    struct sigaction sigact;
    sigemptyset(
        &sigact.sa_mask); // no blocked signals only the one, which arrives
    ...
    setitimer(ITIMER_REAL, &timer,
              NULL); // result = 0, if it is good
                     // when expires, a signal will be sent to the process, and
                     // it restarts
                     // 1. parameter ITIMER_REAL - real time, ITIMER_VIRTUAL
