    ...
    public class TetrisFileDataAccess : ITetrisDataAccess
    {
        ...
        public void Save(string filePath, TetrisPersistence.GameState state)
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(filePath))
                {
                    writer.WriteLine($"{state.Rows} {state.Cols}");
                    
                    writer.WriteLine($"{state.CurrentTetrominoIndex} {state.BlockRow} {state.BlockCol}");
                    
                    writer.Write($"{state.CurrentBlock.Length}");
                    for (int i = 0; i < state.CurrentBlock.Length; i++)
                    {
                        writer.Write($" {state.CurrentBlock[i].row} {state.CurrentBlock[i].col}");
                    }
                    writer.WriteLine();
                    
                    writer.WriteLine(state.PausedTime.TotalSeconds.ToString());
                    
                    writer.WriteLine(state.SaveTime.ToString("o"));
                    
                    for (int i = 0; i < state.Rows; i++)
                    {
                        for (int j = 0; j < state.Cols; j++)
                        {
                            writer.Write($"{state.Board[i, j]} ");
                        }
                        writer.WriteLine();
                    }
                }
            ...
            {
                throw new TetrisDataException("Error writing save file.", ex);
            }
        }
