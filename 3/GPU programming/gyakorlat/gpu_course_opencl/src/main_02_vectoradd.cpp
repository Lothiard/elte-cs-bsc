...
int main() {
  try {
    // Create input vectors of fixed-width integers (int32_t matches OpenCL `int` = 32-bit)
    std::vector<int32_t> A(NUM_ELEMENTS), B(NUM_ELEMENTS);
    std::iota(A.begin(), A.end(), 0);
    std::transform(A.begin(), A.end(), B.begin(),
      [](int32_t x) { return static_cast<int32_t>(NUM_ELEMENTS) - x; });
    ...
    //   Note: Alternatively you may use oclCreateContextBy, useful in a multi-device system
    ...
    const std::string sourceCode = oclReadSourcesFromFile("../kernels/krn_vectoradd.cl");
    ...
    queue.enqueueWriteBuffer(bufferB, CL_TRUE,
      0, sizeof(int32_t) * B.size(), B.data());
    ...
    cl::KernelFunctor<cl::Buffer, cl::Buffer, cl::Buffer> vector_add(program, "vector_add");
    ...
    queue.enqueueReadBuffer(bufferC, CL_TRUE, 0, sizeof(C[0]) * C.size(), C.data());

    // Verify results (print first 10 for sanity)
    for (std::size_t i = 0; i < std::min<std::size_t>(NUM_ELEMENTS, 10); ++i)
      std::cout << A[i] << " + " << B[i] << " = " << C[i] << "\n";

  }
  catch (const cl::Error& err) {
    std::cerr << "OpenCL Error: " << err.what() << " (" << err.err() << "==" << oclErrorString(err.err()) << ")\n";
    return EXIT_FAILURE;
  }
