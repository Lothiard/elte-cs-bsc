...
executeReduction(cl::Context& context, cl::CommandQueue& queue,
    ...
    for (std::size_t num_workitems = size / 2; num_workitems != 0;
         num_workitems >>= 1) {
        int32_t offset = size / (num_workitems * 2);

        cl::Event evt = kernelFunctor(
            cl::EnqueueArgs(queue, cl::NDRange(num_workitems)), buffer, offset);
        evt.wait();
    ...
    int32_t gpu_result{};
    queue.enqueueReadBuffer(buffer, CL_TRUE, 0, sizeof(gpu_result),
                            &gpu_result);
...
void benchmarkReductionKernel(
    const std::string& label, cl::Context& context, cl::CommandQueue& queue,
    cl::KernelFunctor<cl::Buffer, int32_t>& kernelFunctor,
    std::size_t minSize = 1u << 20, std::size_t maxSize = 1u << 27) {
    std::mt19937 rng(1);
    ...
    for (std::size_t size = minSize; size <= maxSize; size <<= 1) {
        ...
        // GPU run
        auto [total_ns, gpu_result] =
            executeReduction(context, queue, kernelFunctor, input);

        double ms = static_cast<double>(total_ns) / 1e6;
        std::cout << "size=" << size << " [" << label << "] time=" << ms
                  << " ms\n";
...
int main() {
    try {
        ...
        const std::string sourceCode =
            oclReadSourcesFromFile("../kernels/krn_reduce_global.cl");
        ...
        // Kernel functors
        cl::KernelFunctor<cl::Buffer, int32_t> reduceAssoc(
            program, "reduce_associative");
        cl::KernelFunctor<cl::Buffer, int32_t> reduceComm(program,
                                                          "reduce_commutative");

        // Run benchmarks
        benchmarkReductionKernel("associative", context, queue, reduceAssoc,
                                 MIN_SIZE, MAX_SIZE);
        benchmarkReductionKernel("commutative", context, queue, reduceComm,
                                 MIN_SIZE, MAX_SIZE);
